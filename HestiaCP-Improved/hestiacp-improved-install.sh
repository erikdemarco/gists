...
if [ $vAddSsh == "y" ] || [ $vAddSsh == "Y" ]; then
    ...
    #setting 'ClientAliveInterval' to low cause connection sometimes hang? so its better to set it to 86400 from previous value 300. edit: the error seems from filezilla, tested with (hestiacp defualt install)
...
#add setting
sed -i -e '/^server_names_hash_bucket_size.*/a autoindex off;' /etc/nginx/nginx.conf
